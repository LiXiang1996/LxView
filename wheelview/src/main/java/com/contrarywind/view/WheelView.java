...
public class WheelView extends View {
    ...
    // Timer mTimer;       ScheduledExecutorService定时周期执行指定的任务
    private ScheduledExecutorService mExecutor = Executors.newSingleThreadScheduledExecutor();
//    ScheduledFuture 很简单，它就是在Future<V>基础上还 集成了 Comparable<Delayed> 和 Delayed 的接口。
//    它用于表示 ScheduledExecutorService 中提交了任务的返回结果
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            textColorCenter =
                    a.getColor(R.styleable.pickerview_wheelview_textColorCenter, 0xFF2a2a2a);
            dividerColor = a.getColor(R.styleable.pickerview_wheelview_dividerColor, 0xFFd5d5d5);
            textSize =
                    a.getDimensionPixelOffset(R.styleable.pickerview_wheelview_textSize, textSize);
            lineSpacingMultiplier =
                    a.getFloat(
                            R.styleable.pickerview_wheelview_lineSpacingMultiplier,
                            lineSpacingMultiplier);
            a.recycle(); // 回收内存
        }
    ...
    public final void scrollBy(float velocityY) { // 滚动惯性的实现
        ...
        mFuture =
                mExecutor.scheduleWithFixedDelay(
                        new InertiaTimerTask(this, velocityY),
                        0,
                        VELOCITY_FLING,
    ...
    public final int getCurrentItem() {
        ...
        if (isLoop && (selectedItem < 0 || selectedItem >= adapter.getItemsCount())) {
            return Math.max(
                    0,
                    Math.min(
                            Math.abs(Math.abs(selectedItem) - adapter.getItemsCount()),
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(
                            contentText, drawOutContentStart, maxTextHeight, paintOuterText);
                    canvas.restore();
                    canvas.save();
                    ...
                    float Y =
                            maxTextHeight
                                    - CENTER_CONTENT_OFFSET; // 因为圆弧角换算的向下取值，导致角度稍微有点偏差，加上画笔的基线会偏上，因此需要偏移量修正一下
                    ...
                    paintOuterText.setTextSkewX(
                            (textXOffset == 0 ? 0 : (textXOffset > 0 ? 1 : -1))
                                    * (angle > 0 ? -1 : 1)
        ...
        switch (event.getAction()) {
            ...
            default:
                if (!eventConsumed) { // 未消费掉事件
                    ...
                    /**
                     * @describe <关于弧长的计算>
                     *     <p>弧长公式： L = α*R 反余弦公式：arccos(cosα) = α 由于之前是有顺时针偏移90度， 所以实际弧度范围α2的值 ：α2
                     *     = π/2-α （α=[0,π] α2 = [-π/2,π/2]） 根据正弦余弦转换公式 cosα = sin(π/2-α) 代入，得： cosα
                     *     = sin(π/2-α) = sinα2 = (R - y) / R 所以弧长 L = arccos(cosα)*R = arccos((R -
                     *     y) / R)*R
                     */
                    float y = event.getY();
                    ...
                    // 已滑动的弧长值
                    mOffset =
                            (int) ((circlePosition - itemsVisible / 2) * itemHeight - extraOffset);
