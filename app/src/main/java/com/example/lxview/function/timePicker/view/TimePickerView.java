...
import com.example.lxview.function.timePicker.configure.PickerOptions;
...
/** 时间选择器 Created by Sai on 15/11/22. Updated by XiaoSong on 2017-2-22. */
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    private void initView(Context context) {
        ...
        if (mPickerOptions.customListener == null) {
            ...
            // 设置文字
            btnSubmit.setText(
                    TextUtils.isEmpty(mPickerOptions.textContentConfirm)
                            ? context.getResources().getString(R.string.pickerview_submit)
                            : mPickerOptions.textContentConfirm);
            btnCancel.setText(
                    TextUtils.isEmpty(mPickerOptions.textContentCancel)
                            ? context.getResources().getString(R.string.pickerview_cancel)
                            : mPickerOptions.textContentCancel);
            tvTitle.setText(
                    TextUtils.isEmpty(mPickerOptions.textContentTitle)
                            ? ""
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        ...
        if (mPickerOptions.startYear != 0
                && mPickerOptions.endYear != 0
        ...
        if (mPickerOptions.startDate != null && mPickerOptions.endDate != null) {
            if (mPickerOptions.startDate.getTimeInMillis()
                    > mPickerOptions.endDate.getTimeInMillis()) {
    ...
    private void initDefaultSelectedDate() {
        ...
        if (mPickerOptions.startDate != null && mPickerOptions.endDate != null) {
            ...
            if (mPickerOptions.date == null
                    || mPickerOptions.date.getTimeInMillis()
                            < mPickerOptions.startDate.getTimeInMillis()
                    || mPickerOptions.date.getTimeInMillis()
                            > mPickerOptions.endDate.getTimeInMillis()) {
    ...
    /** 设置选中时间,默认选中当前时间
     *
     * 可以设置默认的选中时间，比如用户设置过，给默认时间，设置过，给上次设置的时间
     * */
