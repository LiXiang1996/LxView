...
import com.example.lxview.function.timePicker.adapter.NumericWheelAdapter;
import com.example.lxview.function.timePicker.listener.ISelectTimeCallback;
import com.example.lxview.function.timePicker.utils.ChinaDate;
import com.example.lxview.function.timePicker.utils.LunarCalendar;
...
public class WheelTime {
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        wv_year.setAdapter(
                new ArrayWheelAdapter(ChinaDate.getYears(startYear, endYear))); // 设置"年"的显示数据
        ...
        if (ChinaDate.leapMonth(year) == 0) {
            wv_day.setAdapter(
                    new ArrayWheelAdapter(
                            ChinaDate.getLunarDays(ChinaDate.monthDays(year, month))));
        } else {
            wv_day.setAdapter(
                    new ArrayWheelAdapter(ChinaDate.getLunarDays(ChinaDate.leapDays(year))));
        }
        wv_day.setLabel("");
        ...
        // 添加"年"监听
        wv_year.setOnItemSelectedListener(
        ...
        // 添加"月"监听
        wv_month.setOnItemSelectedListener(
                new OnItemSelectedListener() {
                    @Override
                    public void onItemSelected(int index) {
                        int month_num = index;
                        ...
                        int maxItem = 29;
                        if (ChinaDate.leapMonth(year_num) != 0
                                && month_num > ChinaDate.leapMonth(year_num) - 1) {
                            if (wv_month.getCurrentItem() == ChinaDate.leapMonth(year_num) + 1) {
                                wv_day.setAdapter(
                                        new ArrayWheelAdapter(
                                                ChinaDate.getLunarDays(
                                ...
                                wv_day.setAdapter(
                                        new ArrayWheelAdapter(
                                                ChinaDate.getLunarDays(
                                                        ChinaDate.monthDays(year_num, month_num))));
                            ...
                            wv_day.setAdapter(
                                    new ArrayWheelAdapter(
                                            ChinaDate.getLunarDays(
                                                    ChinaDate.monthDays(year_num, month_num + 1))));
